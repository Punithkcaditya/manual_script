<template>
  ...
  <div v-if="isCategoryPage">
    ...
    <ClientOnly>
      <Suspense>
        <template #default>
          <FlatsAvailableSection :flats="flats" :totalFlatCount="totalFlatCountFromCarousel" />
        </template>
        <template #fallback>
          <div class="min-h-[400px] bg-white flex items-center justify-center">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900"></div>
  ...
  <path d="M8.00065 12.6667L3.33398 8.00004L8.00065 3.33337" stroke="#AFA11E" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    <template #fallback>
      <div class="mt-10 flex justify-center items-center min-h-[400px]">
        <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-[#afa11e]"></div>
        <span class="ml-2 text-gray-600">Loading property...</span>
      </div>
    </template>
  </ClientOnly>
...
const flatsOffset = ref(6); // Start at 6 since initial load fetches 6 items (offset 0, limit 6)
...
// Reactive data for category page
const flats = ref([]); // For PropertiesCarousel flats data

// Banner images are now handled via CSS for responsive behavior

// Store totalFlatCount from PropertiesCarousel for FlatsAvailableSection
const totalFlatCountFromCarousel = ref(null);

// Handler for when flats are loaded from PropertiesCarousel
const handleFlatsLoaded = (loadedFlats) => {
  flats.value = loadedFlats;
};

// Handler for when metadata (totalFlatCount) is loaded from PropertiesCarousel
const handleMetadataLoaded = (metadata) => {
  // Store totalFlatCount to pass to FlatsAvailableSection
  if (metadata.totalFlatCount !== null && metadata.totalFlatCount !== undefined) {
    totalFlatCountFromCarousel.value = metadata.totalFlatCount;
  }
};
...
const allFlats = computed(() => {
  if (!componentReady.value || !property.value?.flats) return [];
  ...
  try {
    ...
    if (response.status === "success" && response.flats && Array.isArray(response.flats)) {
      ...
      if (property.value && property.value.flats) {
        // Filter out duplicates before appending - check for flat_id, flatId, or id
        const existingFlatIds = new Set(
          property.value.flats.map(f => f.flat_id || f.flatId || f.id || f.name)
        );
        
        const newFlats = response.flats.filter(flat => {
          const flatIdentifier = flat.flat_id || flat.flatId || flat.id || flat.name;
          return !existingFlatIds.has(flatIdentifier);
        });
        
        // Only append if there are new flats
        if (newFlats.length > 0) {
          property.value.flats = [...property.value.flats, ...newFlats];
        }
      ...
      // Update offset for next load (always increment, even if no new flats)
      flatsOffset.value += 5;
...
const loadFilteredFlats = async () => {
  ...
  try {
    ...
    if (response.status === "success" && response.flats) {
      ...
      // Reset pagination offset (initial load is 6, so next offset starts at 6)
      flatsOffset.value = 6;
    } else {
...
// Reset pagination when filters change - REMOVED offset reset here
// The offset should only be reset when filters change, not when flats are loaded
watch(() => availableFlats.value, () => {
  visibleFlatsCount.value = FLATS_PER_PAGE
  // DO NOT reset offset here - it causes loadMoreFlats to always use the same offset
}, { deep: true });
  ...
  if (property.value) {
    try {
      ...
      if (response.status === "success" && response.flats) {
        ...
        flatsOffset.value = 6; // Reset to 6 after clearing filters (initial load is 6 items)
      }
    } catch (error) {
...
// Call API to resolve friendly URL and get property data
const resolveProperty = async () => {
  try {
    ...
    const authData = {
      ...
      area: area,
      property_slug: propertySlug,
      limit: 6, // Limit flats to 6 for initial load
      // token: 'your-token',
      // userId: 'user-id',
      // etc.
    };
    ...
    if (response.status === "success") {
      ...
      if (response.params?.property_slug || response.name) {
        ...
        property.value = transformedProperty;
        ...
        totalAvailableFlats.value = response.availableFlats || null;
        
        // Reset pagination offset when property is loaded (initial load is 6 items, so next offset starts at 6)
        flatsOffset.value = 6;
        
        // Track property detail page visit
        if (hasCampaignData()) {
          // console.log('📊 Property detail page detected: Campaign ID detected:', campaignId.value)
          // console.log('📊 Property:', propertySlug)
          // console.log('📊 Area:', area)
          // console.log('📊 City:', city)
          // console.log('📊 Is Category Page:', isCategoryPage.value)
          
          // Track as property-detail page since it's an actual property
    ...
    if (categoryInfo) {
      // This is a category page, set flag to render search page instead
      isCategoryPage.value = true;
      categoryType.value = categoryInfo.type;
      categoryValue.value = categoryInfo.value;
      ...
      if (hasCampaignData()) {
        // Track as landing page since it's a category page
        trackPropertyDetailVisit(String(propertySlug), String(area), String(city), 'landing')
      }
      
      // Auto-apply search filters for category page
      const { useUiStore } = await import('~/stores/ui');
      const ui = useUiStore();
...
// Reactive data for nearby places
...
// Reactive data for carousel properties
const carouselProperties = ref([]);
const carouselLoading = ref(true);
const carouselPropertyCount = ref(null); // Store propertyCount from API
...
const loadCarouselProperties = async () => {
  try {
    ...
    // Store propertyCount from API response
    if (apiResponse?.propertyCount !== null && apiResponse?.propertyCount !== undefined) {
      carouselPropertyCount.value = apiResponse.propertyCount;
    }
    
    if (apiResponse?.properties && apiResponse.properties.length > 0) {
      // Transform raw API properties for carousel display
      carouselProperties.value = apiResponse.properties.map((property) => {
        // Use availableFlats count from API response
        const flatsCount = property.availableFlats || 0;

        // Default flat types based on property type (you can customize this)
        let flatTypes = ["Studio", "1 BHK"]; // Default types
      ...
      // No properties from API
      carouselProperties.value = [];
    }

    //console.log("Carousel Properties:", carouselProperties.value);
  } catch (error) {
    // console.error("Error loading carousel properties:", error);
    // Show empty carousel on error
    carouselProperties.value = [];
  } finally {
...
// Call the resolution function first (critical)
try {
...
onMounted(async () => {
  ...
  // Load amenities, carousel properties, and nearby places in the background
  // Nearby places is loaded with lower priority (after a delay to not block initial render)
  Promise.all([
    loadAmenities(),
...
.banner-responsive {
  background-image: url('https://kots-world.b-cdn.net/Final/bannerImages/full/gradient_one-min.webp');
}

@media (max-width: 768px) {
  .banner-responsive {
    background-image: url('https://kots-world.b-cdn.net/Final/bannerImages/full/landing_page_gradient_image_two_1.webp');
  }
}
